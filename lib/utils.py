...
def _compute_loss(y_true, y_predicted, assign_probs):
    # prediction loss
    prediction_loss = masked_mae(y_predicted, y_true, 0.0)

    # balance regularization
    count_per_patch = assign_probs.sum(dim=0)
    mean_count = count_per_patch.mean()
    balance_loss = (count_per_patch - mean_count).abs().mean()  # L1 loss

    loss = prediction_loss + 1.0 * balance_loss
...
def loadData(filepath, metapath, P, Q, train_ratio, test_ratio, tod, dow, log):
    # Traffic
    Traffic = np.load(filepath)['data'][...,:1]
    num_step = Traffic.shape[0]
    # metas
    metas = read_metas(metapath)
    # temporal positions
    ...
    log_string(log, f'Shape of metas: {metas.shape}')
    # train/val/test
    ...
    return trainX, trainY, trainXTE, trainYTE, valX, valY, valXTE, valYTE, testX, testY, testXTE, testYTE, mean, std, metas
