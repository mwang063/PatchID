...
from models.model import PatchID
...
class Solver(object):
    ...
    def __init__(self, config):
        ...
        self.mean, self.std, self.metas = loadData(
                                        self.traffic_file, self.meta_file, self.input_len, self.output_len,
                                        self.train_ratio, self.test_ratio, self.tod, self.dow, log)
        log_string(log, '------------ End -------------\n')
    ...
    def build_model(self):
        self.model = PatchID(self.input_len, self.output_len, self.node_num, self.tod, self.dow,
                            self.layers, self.patch_num, self.input_dims, self.node_dims, self.tod_dims, self.dow_dims).to(self.device)
        ...
        log_string(log, 'The number of parameters: {}.'.format(self.model.param_num()))
    ...
    def vali(self):
        ...
        rmses.append(rmse)
        mapes.append(mape)
        log_string(log, 'average, mae: %.4f, rmse: %.4f, mape: %.4f, val_time: %.4f' % (mae, rmse, mape, time.time() - val_start))
    ...
    def train(self):
        ...
        for epoch in tqdm(range(1,self.max_epoch+1)):
            ...
            train_prediction_l_sum, train_balance_l_sum = 0.0, 0.0
            ...
            for batch_idx in range(num_batch):
                ...
                y_hat, assign_probs = self.model(NormX,TE)

                loss, loss_dict = _compute_loss(Y, y_hat*self.std+self.mean, assign_probs)
                ...
                train_prediction_l_sum += loss_dict['prediction_loss'].cpu().item()
                train_balance_l_sum += loss_dict['balance_loss'].cpu().item()
            ...
            log_string(log, 'epoch %d, lr %.6f, total_loss %.4f, time %.1f sec, prediction_loss %.4f, balance_loss %.4f'
                % (epoch, self.optimizer.param_groups[0]['lr'], train_l_sum / batch_count, epoch_time, train_prediction_l_sum / batch_count, train_balance_l_sum / batch_count))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, help='configuration file', default='./config/SD.conf')
    ...
    parser.add_argument('--patch_num', type=int, default=config['param']['patch_num'])
